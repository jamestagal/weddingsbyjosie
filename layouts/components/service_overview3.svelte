<!--services-section end-->
<section class="ttm-row services-section_1 ttm-bgcolor-grey clearfix">
  <div class="container">
    <!--row-->
    <div class="row">
      <div class="col-lg-7 col-md-10 mx-auto">
        <!--section title-->
        <div class="section-title title-style-center_text without-seperator">
          <div class="title-header">
            <h5>Our Premium Wedding Service</h5>
            <h2 class="title">We Provide The Best Service</h2>
          </div>
          <div class="title-desc">
            <p>
              As a Canberra Marriage Celebrant I conduct, marriage ceremonies,
              same sex couples and also Naming Day and Renewal of Vows
              Ceremonies.
            </p>
          </div>
        </div>
        <!--section title end-->
      </div>
    </div>
    <!--row end-->
    <!--row-->
    <div class="row row-equal-height">
      <div class="col-lg-4 col-md-6">
        <!--featured-icon-box-->
        <div class="featured-icon-box icon-align-top-content style4">
          <div class="featured-icon">
            <div
              class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-lg"
            >
              <i class="ttm-num ti-info" />
              <i class="flaticon flaticon-camera-1" />
            </div>
          </div>
          <div class="featured-content">
            <div class="featured-title">
              <h5>Meet Face-To-Face</h5>
            </div>
            <div class="featured-desc">
              <p>
                We have an inital chat about my service with not obligations.
              </p>
            </div>
          </div>
        </div>
        <!--featured-icon-box end-->
      </div>
      <div class="col-lg-4 col-md-6">
        <!--featured-icon-box-->
        <div class="featured-icon-box icon-align-top-content style4">
          <div class="featured-icon">
            <div
              class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-lg"
            >
              <i class="ttm-num ti-info" />
              <i class="flaticon flaticon-wedding-rings" />
            </div>
          </div>
          <div class="featured-content">
            <div class="featured-title">
              <h5>Book an appointment</h5>
            </div>
            <div class="featured-desc">
              <p>You make a booking with me to lock in the ceremony.</p>
            </div>
          </div>
        </div>
        <!--featured-icon-box end-->
      </div>
      <div class="col-lg-4 col-md-6">
        <!--featured-icon-box-->
        <div class="featured-icon-box icon-align-top-content style4">
          <div class="featured-icon">
            <div
              class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-lg"
            >
              <i class="ttm-num ti-info" />
              <i class="flaticon flaticon-couple" />
            </div>
          </div>
          <div class="featured-content">
            <div class="featured-title">
              <h5>Meet up</h5>
            </div>
            <div class="featured-desc">
              <p>
                We meet up to complete the legals at a minimum of 1 month before
                the ceremony.
              </p>
            </div>
          </div>
        </div>
        <!--featured-icon-box end-->
      </div>
      <div class="col-lg-4 col-md-6">
        <!--featured-icon-box-->
        <div class="featured-icon-box icon-align-top-content style4">
          <div class="featured-icon">
            <div
              class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-lg"
            >
              <i class="ttm-num ti-info" />
              <i class="flaticon flaticon-wine" />
            </div>
          </div>
          <div class="featured-content">
            <div class="featured-title">
              <h5>Have a rehearsal</h5>
            </div>
            <div class="featured-desc">
              <p>
                I attend the rehearsal and run through the procedings so
                everything goes smoothly on the day.
              </p>
            </div>
          </div>
        </div>
        <!--featured-icon-box end-->
      </div>
      <div class="col-lg-4 col-md-6">
        <!--featured-icon-box-->
        <div class="featured-icon-box icon-align-top-content style4">
          <div class="featured-icon">
            <div
              class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-lg"
            >
              <i class="ttm-num ti-info" />
              <i class="flaticon flaticon-church-1" />
            </div>
          </div>
          <div class="featured-content">
            <div class="featured-title">
              <h5>Get you married!</h5>
            </div>
            <div class="featured-desc">
              <p>I marry you according to your wishes on your big day.</p>
            </div>
          </div>
        </div>
        <!--featured-icon-box end-->
      </div>
      <div class="col-lg-4 col-md-6">
        <!--featured-icon-box-->
        <div class="featured-icon-box icon-align-top-content style4">
          <div class="featured-icon">
            <div
              class="ttm-icon ttm-icon_element-onlytxt ttm-icon_element-color-skincolor ttm-icon_element-size-lg"
            >
              <i class="ttm-num ti-info" />
              <i class="flaticon flaticon-romantic-dinner" />
            </div>
          </div>
          <div class="featured-content">
            <div class="featured-title">
              <h5>After the ceremony</h5>
            </div>
            <div class="featured-desc">
              <p>
                I lodge the paperwork to make your wedding legal & let you know
                when the marriage is officially registered.
              </p>
            </div>
          </div>
        </div>
        <!--featured-icon-box end-->
      </div>
    </div>
    <!--row end-->
  </div>
</section>
<!--services-section end-->
